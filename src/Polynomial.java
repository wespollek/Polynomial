...
public class Polynomial extends ArrayList<Integer> {
	...
	StringBuilder str = new StringBuilder() ;
      	...
      	if (current==-1&&size()!=1) str.append('-'); //-x term
        else if (current!=1||size()==1) str.append(current);
	for (int i = size()-1;i>0;i--){
	    ...
	    if(current>0){
		str.append(" + ");
		if(i==1||current!=1) str.append(current);
	    }
	    else if (current<0){
		str.append(" - ");
		if(i==1||current!=-1) str.append(-current);
    ...
    */
	...
	for(int x=0;x<size();x++) if(get(x)!=p.get(x)) return false;
	return true;
    ...
    */
	...
	int degree=0;
        for (; degree<coeffsHighToLow.length-1; degree++)
	    if (coeffsHighToLow[degree]!=0)
		return coeffsHighToLow.length-1-degree;
	return 0;
    }
    ...
    */
	...
	for(int i=0;i<coeffsHighToLow.length;i++){
	    if(coeffsHighToLow[i]==0) insig++;
	    else break;}
	if(insig==coeffsHighToLow.length) return new int []{0};
	int [] coeffsLowToHigh = new int [coeffsHighToLow.length-insig];
	int lastspot=coeffsLowToHigh.length-1;
	for(int i=0;i<=lastspot;i++)
	    coeffsLowToHigh[i]=coeffsHighToLow[coeffsHighToLow.length-1-i];
    ...
    */
	...
	for( int i=coeffsLowToHigh.length-1;i>=0;i--){
	    if(coeffsLowToHigh[i]==0) insig++;
	    else break;}
	if(insig==coeffsLowToHigh.length) return new int[]{0};
	int [] coeffsHighToLow = new int [coeffsLowToHigh.length-insig];
	int lastspot=coeffsHighToLow.length-1;
	for(int i=lastspot;i>=0;i--)
	    coeffsHighToLow[i]=coeffsLowToHigh[lastspot-i];
    ...
    */
	...
	Polynomial little, big;
	if(size()<p.size()){little=this; big=p;}
	else{little=p;big=this;}
	Polynomial sum = new Polynomial ();
	int spot;
	for (spot=0;spot<little.size();spot++)
	    sum.add(spot, little.get(spot)+big.get(spot));
	for (;spot<big.size();spot++)
	    sum.add(spot, big.get(spot));
	while(sum.size()>1 && sum.get(sum.size()-1)==0)
    ...
    */
	...
	Polynomial diff = new Polynomial (new int [p.size()]);
	for(int i=0;i<p.size();i++)
	    diff.add(i, -p.get(i));
	return plus(diff);
